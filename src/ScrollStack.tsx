...
const ScrollStack = ({
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    const endElementTop = endElementOffsetRef.current || (endElement ? getElementOffset(endElement as HTMLElement) : 0);
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      if (blurAmount) {
        ...
        for (let j = 0; j < cardsRef.current.length; j++) {
          ...
          if (!jCard) continue;
          const jCardTop = cardOffsetsRef.current[j] || getElementOffset(jCard);
      ...
      const newTransform: TransformCache = {
  ...
  useLayoutEffect(() => {
    ...
    if (useWindowScroll) {
      ...
      lenis = setupLenis() as Lenis | null;
    }
